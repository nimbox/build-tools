#!/usr/bin/env bash
#
# bump.sh — the Nimbox ship command.
#
# Single source of truth: the repo-root `VERSION` file. A `version.json`
# manifest declares where that value must be stamped (the "targets") and which
# branch releases ship from. The main command performs the whole guarded
# release: bump VERSION, stamp every target, commit, tag, and push exactly
# that commit and tag atomically. CI (the reusable release workflows) runs
# `verify` against the pushed tag before publishing.
#
# usage: bump <major|minor|patch|X.Y.Z>   guarded release (see below)
#        bump current                     print the version (from VERSION)
#        bump apply                       stamp VERSION into every target
#        bump verify [<tag>]              assert targets (and tag) == VERSION
#
# The release pipeline:
#   1. guards — on the release branch; working tree clean; after a fetch,
#      the local branch exactly matches its origin counterpart
#   2. release commit — bump VERSION, stamp targets, commit "Release x.y.z"
#   3. tag + atomic push — tag vx.y.z, then push branch and tag together
#      (--atomic): both land or neither does; the guards prove nothing else
#      can be pushed
#
...
# thin `scripts/bump` bootstrap and carry only their own VERSION and
# version.json.
...
cmd_release() {
    ...
    [ "$current_branch" = "$branch" ] || die "not on $branch (on \"${current_branch:-detached HEAD}\")"
    [ -z "$(git status --porcelain)" ] || die "working tree not clean; commit or stash first"
    git fetch --quiet origin "$branch" || die "could not fetch origin/$branch"
    [ "$(git rev-parse HEAD)" = "$(git rev-parse "origin/$branch")" ] \
        || die "local $branch does not match origin/$branch; sync first (git pull / git push your work through review)"

    # 2. release commit
    local version major minor patch new_version
    version="$(read_version)"
    case "$part" in
        major|minor|patch)
            [[ "$version" =~ $SEMVER_RE ]] || die "$VERSION_FILE is not a semver: \"$version\""
            ...
            ;;
        *)
            [[ "$part" =~ $SEMVER_RE ]] || die "not major/minor/patch or a semver (X.Y.Z): \"$part\""
            new_version="$part"
            ;;
    esac
    ...
    VERIFY_FAILURES=0
    for_each_target _verify_one
    [ "$VERIFY_FAILURES" -eq 0 ] || die "targets did not verify after apply"

    git commit --quiet -am "Release $new_version"

    # 3. tag + atomic push
    git tag -a "$tag" -m "Release $new_version"
    git push --atomic origin "$branch" "$tag"
    echo "released $new_version: pushed $branch and $tag"
}
...
usage: bump <command>
  ...
  major | minor | patch | <X.Y.Z>
                        guarded release: assert on the release branch, clean,
                        and in sync with origin; bump VERSION, stamp targets,
                        commit "Release X.Y.Z", tag, push branch + tag
                        atomically. CI publishes from the tag.
...
case "$cmd" in
    ...
    apply)   cmd_apply "$@" ;;
    verify)  cmd_verify "$@" ;;
    major|minor|patch) cmd_release "$cmd" ;;
    [0-9]*)  cmd_release "$cmd" ;;
    ""|-h|--help|help) usage ;;
